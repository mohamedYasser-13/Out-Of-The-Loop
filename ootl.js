...
function getRoundsNumber(roundMood){
  ...
  <div class="category-selection">
    ...
    <div onclick="getCategory('vegetables');"><p>Vegetables & Fruits</p></div>
  </div>
  `
}
...
function getCategory(roundCategory){
  ...
  if(category === 'animals'){
    ...
    playList = vegetablesAndFruitsList.slice();
  }
   if(mood !== 'teams'){
    startRound(); 
  } else {
...
function giveChoices(){
  ...
  let choicesList = playList.slice(0, 8);
  choicesList.push(theWord);
  choicesList = shuffle(choicesList);
  let choices = ``;
  let wordChosen;
  for(i=0; i<9; i++){
    choices += `<button class="choice${i}" onclick="wordChosen = '${choicesList[i]}'; if(checks){checkIt(wordChosen); checks--;}">${choicesList[i]}</button>`
  }
  main.innerHTML = `
  <div class="choice">
